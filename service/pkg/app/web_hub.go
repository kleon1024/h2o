...
func (s *Server) HubRegister(wsConnect *WebConn) {
	logrus.Info("HubRegister", wsConnect.UserId)
	hub := s.GetHubForUserId(wsConnect.UserId)
	if hub != nil {
		hub.Register(wsConnect)
	}
}
...
func (h *Hub) Register(wsConnect *WebConn) {
	select {
	case h.register <- wsConnect:
	case <-h.stop:
	}
}

func (h *Hub) Unregister(wsConnect *WebConn) {
	select {
	case h.unregister <- wsConnect:
	case <-h.stop:
	}
}
...
func (h *Hub) Start() {
	...
	doRecover = func() {
		if r := recover(); r != nil {
			...
			logrus.Error("WsHub stopped unexpectedly. Recovering.")
		}
