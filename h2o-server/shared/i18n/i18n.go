...
func getTranslationsBySystemLocale() (TranslateFunc, error) {
	...
	if _, ok := locales[locale]; !ok {
		logrus.WithField("locale", locale).WithField("attempting to fall back to default locale", defaultLocale).Warn("Failed to load system translations for")
	...
	logrus.WithField("for locale", locale).WithField("from locale", locales[locale]).Info("Loaded system translations")
...
func escapeForHTML(arg interface{}) interface{} {
	...
	default:
		...
		logrus.WithField("html_template", arg).
			WithField("template_type", reflect.ValueOf(arg).Type().String()).
			Warn("Unable to escape value for HTML template")
		return ""
	}
}
